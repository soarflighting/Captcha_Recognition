...
#生成验证码图片
def generate_captcha_text_image():
    image = ImageCaptcha()
    captcha_text = random_captcha_text()
    captcha_text="".join(captcha_text)
    captcha = image.generate(captcha_text)
    #image.write(captcha_text,"checkcode/"+captcha_text+".png")
...
def get_next_batch(batch_size = 128):
    ...
    def wrap_gen_captcha_text_and_image():
        while True:
            text,image = generate_captcha_text_image()
...
def cnn_structure(w_alpha = 0.01,b_alpha = 0.1):
    ...
    # conv3 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
...
if __name__ == '__main__':
    ...
    if train ==0:
        print("模型训练中...")
        text,image = generate_captcha_text_image()
        print("验证码大小：",image.shape)

        image_height = 60
        image_width = 160
        max_captcha =len(text)
        print("验证码字符数：",max_captcha)
        char_set = number
